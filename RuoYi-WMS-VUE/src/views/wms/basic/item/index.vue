  ...
  <div class="app-container">
    <el-card>
      <el-form :model="queryParams" ref="queryFormRef" :inline="true" label-width="68px">
        <el-form-item label="原料名称" prop="itemName">
          <el-input v-model="queryParams.itemName" placeholder="请输入原料名称" clearable @keyup.enter="handleQuery"/>
        </el-form-item>
        <el-form-item label="原料品牌" prop="itemBrand">
      ...
      <div style="display: flex;align-items: start">
        <div>
          <div style="display: flex;align-items: center;justify-content: space-between">
            <span style="font-size: 18px;line-height: 18px">原料分类</span>
        ...
        <div style="width: 100%;position: relative">
          <div style="display: flex;align-items: start;justify-content: space-between">
            <span class="mr10" style="font-size: 18px;">原料列表</span>
            <el-button type="primary" plain icon="Plus" @click="handleAdd" class="mb10">新增原料</el-button>
          </div>
          <el-table :data="itemList" @selection-change="handleSelectionChange" :span-method="spanMethod" border empty-text="暂无原料" v-loading="loading" cell-class-name="my-cell">
            <el-table-column label="原料信息" prop="itemId">
            ...
            <el-table-column label="重量(kg)" width="160" align="left">
              <template #default="{ row }">
                <div v-if="row.itemSku.netWeight" class="flex-space-between">
                  <span>净重：</span>
                  <div>{{ row.itemSku.netWeight }}</div>
                </div>
                <div v-if="row.itemSku.grossWeight" class="flex-space-between">
                  <span>毛重：</span>
                  <div>{{ row.itemSku.grossWeight }}</div>
                </div>
              </template>
            </el-table-column>
        ...
        <el-card>
          <el-form ref="itemFormRef" :model="form" :rules="rules" label-width="108px">
            <el-row :gutter="24">
              ...
              <el-col :span="10">
                <el-form-item label="原料分类" prop="itemCategory">
                  <el-tree-select
                    ref="treeRef"
        ...
        <el-card class="mt20">
          ...
          <el-form :model="skuForm" :rules="skuRules" ref="skuFormRef" :show-message="false">
            <el-table :data="skuForm.itemSkuList" border cell-class-name="my-cell">
              ...
              <el-table-column label="净重/毛重(kg)" width="200">
                <template #default="scope">
              ...
              <template #append v-if="skuForm.itemSkuList.length">
                <div style="padding: 6px 2px 6px 2px;text-align: center;">
                  <el-button text class="add-btn" icon="Plus" type="primary" @click="onAppendBtnClick">添加原料规格
                  </el-button>
                </div>
              </template>
              <template #empty>
    ...
    <el-dialog :title="categoryDialog.title" v-model="categoryDialog.visible" width="500px" append-to-body
      ...
      <el-form ref="itemCategoryFormRef" :model="categoryForm" :rules="typeRules" label-width="128px" @submit.native.prevent>
        ...
        <el-form-item label="原料分类名称" prop="categoryName">
          <el-input v-model="categoryForm.categoryName" placeholder="请输入原料分类名称" @keyup.enter="submitCategoryForm"/>
        </el-form-item>
      </el-form>
      <template #footer>
        <div class="dialog-footer">
...
const categoryData = reactive({
  ...
  rules: {
    id: [
      {required: true, message: "原料分类id不能为空", trigger: "blur"}
    ],
    // parentId: [
    //   {required: true, message: "父原料类型id不能为空", trigger: "blur"}
    // ],
    categoryName: [
      {required: true, message: "原料分类名称不能为空", trigger: "blur"}
    ],
    status: [
      {required: true, message: "原料分类状态不能为空", trigger: "change"}
    ],
  }
});
...
const handleDeleteItemSku = async (row, index) => {
  ...
  if (skuForm.itemSkuList.length === 1) {
    return proxy?.$modal.msgError("至少包含一个原料规格");
  }
  ...
  itemFormRef.value.validate(async (valid) => {
    if (valid) {
      ...
      if (!skuForm.itemSkuList || skuForm.itemSkuList.length === 0) {
        proxy?.$modal.msgError("至少包含一个原料规格");
        flag = false
      }
...
const handleDelete = async (row) => {
  ...
  await proxy?.$modal.confirm('确认删除原料【' + row?.item.itemName + '】吗？');
  loading.value = true;
